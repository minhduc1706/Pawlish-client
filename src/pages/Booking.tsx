...
import { createVNPayServicePayment } from "@/api/payment";
...
export default function BookingPage() {
  ...
  const { data: serviceCategories, isLoading: isServiceCategoryLoading } =
    useServiceCategory();
  ...
  useEffect(() => {
    ...
    const appointmentId = queryParams.get("vnp_TxnRef");
    const success = queryParams.get("success");

    if (responseCode || success) {
      if (responseCode === "00" || success === "true") {
        setPaymentStatus(`Thanh toán thành công cho lịch hẹn ${appointmentId}`);
  ...
  const handleContactInfoChange = (field: keyof UserType, value: string) => {
    ...
    // Cập nhật localStorage nếu cần (tùy chọn)
    const updatedUser = { ...contactInfo, [field]: value };
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const paymentData = {
        ...
        time: timeSlot,
        pet: newPet._id || "",
        user: contactInfo._id,
        notes: notes || undefined,
      };
      console.log("Payment data:", paymentData);

      const paymentUrl = await createVNPayServicePayment(paymentData);
  ...
  const renderServiceSelection = () => {
    ...
    const filteredServices =
      selectedCategoryId && selectedCategoryId !== "all"
        ? services?.filter(
            (service) => service.category_id._id === selectedCategoryId
          )
        : services;
  ...
  const renderStaffSelection = () => {
    ...
    const availableStaff =
      ...
      staffs?.filter((staff) =>
        staff.service_id?.some(
          (service: Service) => service._id === selectedService?._id
        )
      );
        ...
        <div className="space-y-6">
          <div className="space-y-2 text-center">
            ...
            <p className="text-muted-foreground">
              Không có nhân viên nào cho dịch vụ {selectedService?.name}.
            </p>
          </div>
    ...
    return (
      <div className="space-y-6">
        <div className="space-y-2 text-center md:text-left">
          <h2 className="text-2xl font-bold">Chọn nhân viên</h2>
          <p className="text-muted-foreground">
            Chọn người sẽ chăm sóc thú cưng của bạn cho dịch vụ{" "}
  ...
  const renderDateTimeSelection = () => (
    <div className="space-y-6">
      <div className="space-y-2 text-center md:text-left">
        ...
        <p className="text-muted-foreground">
          Chọn thời gian bạn muốn đưa thú cưng đến.
        </p>
      </div>
  ...
  const renderPetInfoForm = () => (
    <div className="space-y-6">
      <div className="space-y-2 text-center md:text-left">
        ...
        <p className="text-muted-foreground">
          Cho chúng tôi biết về thú cưng của bạn để chúng tôi có thể chăm sóc
          tốt nhất.
        </p>
      </div>
        ...
        <CardContent className="pt-6">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value) => handlePetInfoChange("species", value)}
            ...
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value) => handlePetInfoChange("gender", value)}
    ...
    return (
      <div className="space-y-6">
        <div className="space-y-2 text-center md:text-left">
          ...
          <p className="text-muted-foreground">
            Vui lòng kiểm tra hoặc cập nhật thông tin liên hệ của bạn.
          </p>
        </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-2 md:col-span-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleContactInfoChange("full_name", e.target.value)
                  }
        ...
        <div className="flex justify-between pt-4">
          ...
          <Button
            onClick={handleNextStep}
            disabled={
              !contactInfo.full_name || !contactInfo.email || !contactInfo.phone
            }
            className="transition-all duration-200 hover:scale-105"
            size="lg"
          >
  ...
  const renderPaymentForm = () => (
    <div className="space-y-6">
      <div className="space-y-2 text-center md:text-left">
        ...
        <p className="text-muted-foreground">
          Hoàn tất đặt lịch bằng cách thanh toán.
        </p>
      </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">
                  {selectedService?.category_id.name}
                </p>
              </div>
              <div>
    ...
    <div className="space-y-8">
      <div className="flex flex-col items-center justify-center">
        ...
        <p className="text-muted-foreground text-center mt-2">
          Cảm ơn bạn đã đặt lịch tại PetSpa. Chúng tôi đã gửi email xác nhận đến{" "}
          {contactInfo.email}.
        </p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-lg p-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-start space-x-3">
            ...
            <p className="text-blue-700">
              Nhớ mang theo hồ sơ tiêm chủng của thú cưng nếu đây là lần đầu
              tiên bạn đến.
            </p>
          </div>
        </div>
